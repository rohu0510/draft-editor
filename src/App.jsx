...
const TextEditor = () => {
  ...
  const handleBeforeInput = (inputChar, currentEditorState) => {
    ...
    const textBefore = currentBlock.getText().slice(0, selection.getStartOffset());
    ...
    if (textBefore === '#') {
      ...
      newEditorState = EditorState.push(currentEditorState, newContent, 'change-block-type');
    ...
    if (textBefore === '***') {
      ...
      newEditorState = EditorState.push(currentEditorState, newContent, 'change-inline-style');
      newEditorState = removeAllInlineStyles(newEditorState, blockKey);
  ...
  const handleReturn = (e, currentEditorState) => {
    ...
    const newEditorState = EditorState.push(currentEditorState, newContentStateWithUnstyled, 'split-block');
    setEditorState(newEditorState);
    return 'handled';
  };
